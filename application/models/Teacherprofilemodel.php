...
Class Teacherprofilemodel extends CI_Model
{
  ...
  function getuser($user_id,$user_type)
   {

     $query="SELECT ed.*,et.* FROM edu_users AS ed LEFT JOIN edu_staff_details AS et ON ed.user_master_id=et.id WHERE ed.user_id='$user_id' AND ed.user_type='$user_type' AND et.role_type=ed.user_type";
  ...
  function teacherprofileupdate($user_id,$userFileName,$user_type,$users_id)
    {
	   $query="UPDATE edu_staff_details SET updated_at=NOW(),updated_by='$users_id' WHERE id='$user_id'";

      $query1="UPDATE edu_users SET user_pic='$userFileName',updated_date=NOW() WHERE user_master_id='$user_id' AND user_type='$user_type'";
