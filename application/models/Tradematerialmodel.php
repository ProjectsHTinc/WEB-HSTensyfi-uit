...
Class Tradematerialmodel extends CI_Model
{
...
//GET ALL Years
        ...
        function get_cur_year(){
          ...
          function get_all_active_trade(){
            $acd_year=$this->get_cur_year();
            $year_id= $acd_year['cur_year'];
            $get_alltrade="SELECT * FROM edu_trade  WHERE status='Active' AND year_id='$year_id'";
            $res=$this->db->query($get_alltrade);
            return $res->result();
          }
       //GET ALL TERMS

       function checking_title($trade_title){
        $acd_year=$this->get_cur_year();
        $year_id= $acd_year['cur_year'];
