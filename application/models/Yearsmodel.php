...
Class Yearsmodel extends CI_Model
{
   ...
   function add_years($formatted_date,$formatted_date1,$status,$user_id)
   {
      ...
      if ($fy < $ty && $fy != $ty) 
      {
         ...
         $query     = "INSERT INTO edu_academic_year(from_month,to_month,status,created_at,created_by)VALUES('$formatted_date','$formatted_date1','$status',NOW(),'$user_id')";
    ...
    function update_years($year_id,$formatted_date,$formatted_date1,$status,$user_id)
    {
     ...
     if ($fy < $ty && $fy = $ty) {
      ...
      if($result->num_rows()==0)
      { */
      $query = "UPDATE edu_academic_year SET from_month='$formatted_date',to_month='$formatted_date1',status='$status',updated_at=NOW(),updated_by='$user_id' WHERE year_id='$year_id'";
